...
shared$LWS.toSafeMap(new shared$LWS.MapCtor());
...
function proxyMaskFunctionDistortion$LWS({
  ...
  if (LOCKER_VERBOSE_INSTRUMENTATION_FLAG$LWS) {
    ...
    activityName$LWS = factoryName$LWS ? shared$LWS.ReflectApply(shared$LWS.StringProtoReplace, factoryName$LWS, [shared$LWS.LOCKER_IDENTIFIER_MARKER, '']) : '<unknown>';
  }
...
function createResourceLoadError$LWS(url$LWS) {
  return new shared$LWS.LockerSecurityError(`platformResourceLoader error loading ${shared$LWS.enquote(url$LWS)}.`);
}
...
function createEvalHelpersFactoryArgs$LWS(record$LWS) {
  ...
  function loadScript$LWS(_thisArg$LWS, url$LWS) {
    ...
    const resolvedURL$LWS = sharedUrl$LWS.resolveURL(urlAsString$LWS);
    ...
    for (let i$LWS = 0, {
      ...
      // istanbul ignore else: current tests have no way of expressing a state that would cause this condition to evaluate false
      if (status$LWS && shared$LWS.ReflectApply(scriptSrcGetterDistortion$LWS, script$LWS, []) === resolvedURL$LWS) {
        ...
        if (status$LWS === 1 /* ResourceLoaderStatus.Errored */) {
          return shared$LWS.PromiseReject(createResourceLoadError$LWS(urlAsString$LWS));
        }
    ...
    shared$LWS.ReflectApply(scriptSrcSetterDistortion$LWS, script$LWS, [urlAsString$LWS]);
  ...
  function loadStyle$LWS(_thisArg$LWS, url$LWS) {
    const urlAsString$LWS = shared$LWS.toString(url$LWS);
    let link$LWS = shared$LWS.ReflectApply(sharedDom$LWS.ElementProtoQuerySelector, head$LWS, [`link[href=${shared$LWS.enquote(urlAsString$LWS)}]`]);
    if (link$LWS) {
