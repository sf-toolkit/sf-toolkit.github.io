...
export function matchRouteDefinitionByPageReference(pageReference, routeDefs) {
  ...
  if (refType) {
    ...
    const [matched] = routeDefs.filter(compiledDef => {
      ...
      // pageReference type matches page binding type
      const matchesType = type === refType;
      // to match this route, pageReference attribute keys match all the keys in page binding attributes
      ...
      // to match this route, the pageReference state keys should
...
export function getPathParams(path, routeDef) {
  ...
  if (execArray) {
    ...
    // For each defined param, match it up with the path value
    const pathParams = {};
