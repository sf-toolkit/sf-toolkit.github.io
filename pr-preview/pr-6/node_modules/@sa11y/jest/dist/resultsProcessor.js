...
function createA11yTestResult(testResult, a11yResult) {
    return {
        ...
        failureMessages: [
            `${common_1.errMsgHeader}: ${a11yResult.description}
 ...
 */
function processA11yErrors(testSuite, testResult) {
    ...
    testResult.failureDetails.forEach((failure) => {
        ...
        if (error.name === format_1.A11yError.name) {
            ...
            format_1.A11yResults.add(error.a11yResults, suiteName).forEach((a11yResult) => {
                const className = `${suiteName}${metadataSeparator}${a11yResult.wcag}`;
                // TODO (code cov): Fix - should be covered by existing tests
                /* istanbul ignore next */
                if (!Array.isArray(consolidatedErrors.get(className)))
                    consolidatedErrors.set(className, []);
 ...
 */
function modifyOriginalTestResult(results, testSuite, testResult) {
...
function resultsProcessor(results) {
    ...
    (0, common_1.log)(`Transforming a11y failures from ${consolidatedErrors.size} suites ..`);
    consolidatedErrors.forEach((testResults, suiteKey) => {
        const sa11ySuite = (0, test_result_1.createEmptyTestResult)();
        ...
        (0, test_result_1.addResult)(results, sa11ySuite);
    });
    return results;
}
exports.default = resultsProcessor;
