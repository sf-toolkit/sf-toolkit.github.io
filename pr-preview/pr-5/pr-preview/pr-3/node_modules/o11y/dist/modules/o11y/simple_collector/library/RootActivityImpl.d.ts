import { RootActivity, Schema } from '../interfaces';
import { ActivityDetail } from '../interfaces/ActivityDetail';
import { ActivityImpl, onActivityError, onActivityStop } from './ActivityImpl';
import { SchematizedData } from 'o11y/shared';
export declare class RootActivityImpl extends ActivityImpl implements RootActivity {
    constructor(name: string, onStopped: onActivityStop, onError: onActivityError, rootId?: string, isSampled?: boolean, startPerfTime?: number, generatedRootIdLength?: number);
    private _preRootId;
    get preRootId(): string;
    set preRootId(value: string);
    protected _getDetail(userSchema: Schema, userData: SchematizedData): ActivityDetail;
}
