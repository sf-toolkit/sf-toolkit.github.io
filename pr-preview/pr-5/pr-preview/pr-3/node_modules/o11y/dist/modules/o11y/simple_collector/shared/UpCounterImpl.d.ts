import { Metric } from '../interfaces/Metric';
import { UpCounter } from '../interfaces/UpCounter';
import type { MetricsTags } from './TypeDefinitions';
export declare class UpCounterImpl implements Metric<number>, UpCounter {
    private readonly _name;
    private readonly _ownerName;
    private readonly _ownerAppName;
    private readonly _tags;
    private readonly _createdOn;
    private _lastUpdatedOn;
    private _value;
    constructor(_name: string, _ownerName: string, _ownerAppName: string, _tags: MetricsTags, createdOn?: number, lastUpdatedOn?: number, value?: number);
    getName(): string;
    getCreatedOn(): number;
    getLastUpdatedOn(): number;
    getData(): number;
    increment(value?: number): void;
    reset(): void;
    getOwnerName(): string;
    getOwnerAppName(): string;
    getTags(): Readonly<MetricsTags>;
}
