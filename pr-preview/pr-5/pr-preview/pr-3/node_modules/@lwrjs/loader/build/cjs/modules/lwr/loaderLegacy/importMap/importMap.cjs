...
function applyPackages(id, packages, defaultUri) {
  ...
  if (pkgName) {
    ...
    if (id.length > pkgName.length && pkg[pkg.length - 1] !== "/") {
      ...
      if (isPackage) {
        const uri2 = pkg.substring(0, pkg.lastIndexOf(pkgName)) + encodeURIComponent(id);
        return {uri: uri2};
      }
...
function resolveImportMapEntry(importMap, resolvedOrPlain, parentUrl) {
  ...
  return applyPackages(resolvedOrPlain, importMap.imports, importMap.default) || (0, import_url.isUrl)(resolvedOrPlain) && {uri: resolvedOrPlain} || void 0;
}
